...
describe('validate', () => {
    describe('string', () => {
        ...
        it('should not throw an error on empty string target with empty flat to false', () => {
            const name = 'something'
    ...
    describe('type', () => {
        it('should succeed on matchig non-primitive type', () => {
            const name = 'something'
            ...
            target = 'true', type = Boolean
            expect(() => validate.type(target, name, type)).to.throw(TypeError, `${name} ${target} is not a ${type.name.toLocaleLowerCase()}`)
        })        
    })
})