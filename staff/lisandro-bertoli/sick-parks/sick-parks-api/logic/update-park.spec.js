...
const { mongoose, models: { Park, User, Location, Feature } } = require('sick-parks-data')
    ...
    describe('when user and park exist', () => {
        ...
        describe('when user is the creator', () => {
            ...
            beforeEach(async () => {
                ...
                newSize = 'xl'
                newLevel = 'advanced'
                newLocation = new Location({ coordinates: [random() * 15 + 1, random() * 15 + 1] })
                features = [feature]
            })

            it('should succeed on valid fields', async () => {
                await updatePark(userId, parkId, { name: newName, location: newLocation, size: newSize, level: newLevel, features })
                ...
                expect(park.location.coordinates).to.deep.equal(newLocation.coordinates)


                expect(park.features[0].location.coordinates).to.deep.equal(feature.location.coordinates)
