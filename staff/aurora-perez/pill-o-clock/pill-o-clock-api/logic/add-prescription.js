...
module.exports = (id, drugName, time) => {
    ...
    return Promise.all([User.findById(id).lean(), Drug.findOne({drugName}).lean() ])
        ...
        .then (user => {
            ...
            const guideline = new Guideline({created: new Date, prescribed: user.id, drug: _drug.id, times: [time]})

            return Promise.all([User.findByIdAndUpdate(user.id, { $push: { prescription: guideline } }), guideline.save()])
