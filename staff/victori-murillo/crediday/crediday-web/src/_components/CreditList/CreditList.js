...
import getDay from '../../_utils/getDay'
...
import convertWhatsapp from '../../_utils/convertWhatsapp'
...
export default class List extends React.Component {
  ...
  getCredits = () => {
    ...
    // .then(result => this.setState({credits: result.data.credits}))
    .then((result) => {
      let {credits} = result.data
  ...
  getBalance = credit => {
    ...
    return (amount - totalAmort) + interestPending + moratoriumPending
  }
  ...
  render() {
    ...
    // if (credits[0]) {
    //   validation = credits[0].length === 0 ? false : true
    // }


    // allEmpty has to be FALSE
    const allEmpty = credits.every(c => c.length === 0)


    if ( (credits.length > 0 && isAdmin()) && !allEmpty ) {

      totalInvestments = (credits.length > 0) ? credits.filter(c => c.payments).map(c => {
      ...
      <div className="container_list" >
...

        {
          ...
          <Fragment>
            <div className="titles-list" >
              <span>Cantidad de Inversiones: {porCobrar? credits.length : 0}</span>
              <span>Capital Invertido: ₡{addPoints(totalInvestments)}</span>
            </div>
            <hr />
          </Fragment>
        }
        ...
        
        {
          ...
          <div id="btns-frecuncy" className="days-buttons" >
            ...
            {
              this.state.credits.filter(c => c.creditState).some(c => c.paymentFrecuency === "Daily") &&
              <button 
              name="Daily"
            ...
            {
              this.state.credits.filter(c => c.creditState).some(c => c.paymentFrecuency === "Weekly") &&
              <button 
              name="Weekly"
              onClick={this.filterFrecuency} 
            ...
            {
              this.state.credits.filter(c => c.creditState).some(c => c.paymentFrecuency === "TwiceMonthly") &&
              <button 
              name="TwiceMonthly" 
              onClick={this.filterFrecuency} 
            ...
            {
              this.state.credits.filter(c => c.creditState).some(c => c.paymentFrecuency === "Monthly") &&
              <button 
              name="Monthly" 
              onClick={this.filterFrecuency} 
                  ...
                  <tr key={i} >
                    ...
                    <td>
                      <button
                        ...
                        name="displayPayment"
                        className="payment-today" >{addPoints(c.paymentInterest + c.paymentAmortize)}
                      </button>
                    </td>
                    ...
                    }
                    {
                      ...
                      <td className="column_amount" >
                        <button
                          onClick={(e) => this.openDisplay(e, c)}
                          name="displayBalance"
                          className="button_balance" id="balance-list" >{addPoints(this.getBalance(c))}</button>
                      </td>
                    }
                  </tr>
                )
