...
const confirmData = require('./confirm-data')
...
describe('confirmData', () => {
  ...
  describe('When company and username already exists', () => {
    ...
    it('Should succeed confirming company', async () => {
      const { message } = await confirmData({ company: companyName, email })
      expect(message).to.be.a('string')
      expect(message).to.equal('Data confirmed')
    })

    it('Should fail with syntax error', () => {
      try {
        confirmData({ company: 123 })
        throw new Error('should not reach this point')

      } catch (error) {
        expect(error.message).to.equal('Nombre de Compañia 123 is not a string')
      }
    })

    it('Should fail with wrong companyId', async () => {
      try {
        await confirmData({ company: 'wrong-company', email })
        throw new Error('should not reach this point')
      } catch (error) {
        expect(error.message).to.equal('Nombre de compañia no coincide')
      }
    })
