//TODO
import deleteToilet from './delete-toilet'
...
describe('deleteToilet', () => {
    ...
    describe('happy paths', () => {
        ...
        it('should successfully delete a toilet post', async () => {
            ...
            let deleted = await deleteToilet(token, _toiletId)
        ...
        it('should fail to delete the toilet if the user is deactivated', async () => {
            ...
            try {
                await deleteToilet(token, _toiletId)
            } catch (error) {
                _error = error
            }
        ...
        it('should fail to delete the toilet on an invalid token', async () => {
            let _error
            try {
                await deleteToilet(`${token}-wrong`, _toiletId)
            } catch (error) {
                _error = error
            }
        ...
        it('should fail to delete the toilet if the user does not exist', async () => {
            ...
            let _error
            try {
                await deleteToilet(token, _toiletId, _commentId)
            } catch (error) {
                _error = error
            }
            expect(_error).toBeDefined()
    ...
    describe('unhappy paths', () => {
        it('should fail on a non string token', () => {
            token = 45438
            expect(() => deleteToilet(token, _toiletId)).toThrow(`token ${token} is not a string`)
            ...
            token = undefined
            expect(() => deleteToilet(token, _toiletId)).toThrow(`token is empty`)
        })

        it('should fail on a non-string toilet ID', () => {
            token = 'some token'
            _toiletId = 45438
            expect(() => deleteToilet(token, _toiletId)).toThrow(`toiletId ${_toiletId} is not a string`)
            ...
            _toiletId = undefined
            expect(() => deleteToilet(token, _toiletId)).toThrow(`toiletId is empty`)
        })
    })
