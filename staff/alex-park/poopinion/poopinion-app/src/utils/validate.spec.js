...
describe('validate', () => {
    describe('string', () => {
        it('should not throw on string target', () => {
            ...
            expect(() => validate.stringFrontend(target, name)).not.to.throw()
        })
        ...
        it('should throw type-error on non-string target', () => {
            ...
            let target = 1
            expect(() => validate.stringFrontend(target, name)).to.throw(TypeError, `${name} ${target} is not a string`)
        ...
        it('should throw type-error on empty string target with default empty flat to true', () => {
            ...
            let target = ''
            expect(() => validate.stringFrontend(target, name)).to.throw(Error, `${name} is empty`)
        })
        ...
        it('should not throw on empty string target with empty flat to false', () => {
            ...
            let target = ''
            expect(() => validate.stringFrontend(target, name, false)).not.to.throw()
        })
    })
